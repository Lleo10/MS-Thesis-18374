...
def force_pull(fx,fy,fz,x,y,z,rc2):
    
    x_diff_tan1=x[selected_mono1]-x[selected_mono1-1]
    y_diff_tan1=y[selected_mono1]-y[selected_mono1-1]
    z_diff_tan1=z[selected_mono1]-z[selected_mono1-1]
    
    x_diff_tan2=x[selected_mono2]-x[selected_mono2+1]
    y_diff_tan2=y[selected_mono2]-y[selected_mono2+1]
    z_diff_tan2=z[selected_mono2]-z[selected_mono2+1]
    
    r2_1=((x_diff_tan1**2)+(y_diff_tan1**2)+(z_diff_tan1**2))
    r_1 = np.sqrt(r2_1)
    
    r2_2=((x_diff_tan2**2)+(y_diff_tan2**2)+(z_diff_tan2**2))
    r_2 = np.sqrt(r2_2)
    
    force1 = (force_strength/r_1)
    fox1 = force1*x_diff_tan1 
    foy1 = force1*y_diff_tan1 
    foz1 = force1*z_diff_tan1 
    
    force2 = (force_strength/r_2)
    fox2 = force2*x_diff_tan2 
    foy2 = force2*y_diff_tan2 
    foz2 = force2*z_diff_tan2     
    
    fx[selected_mono1-1] = fx[selected_mono1-1] + fox1
    fy[selected_mono1-1] = fy[selected_mono1-1] + foy1
    fz[selected_mono1-1] = fz[selected_mono1-1] + foz1

    fx[selected_mono2+1] = fx[selected_mono2+1] + fox2
    fy[selected_mono2+1] = fy[selected_mono2+1] + foy2
    fz[selected_mono2+1] = fz[selected_mono2+1] + foz2
    
    return fx,fy,fz
    
def distance_check_1(x,y,z,xd,yd,zd,selected_mono1,i):
    x_dist_1 = x[selected_mono1] - xd[0]
    y_dist_1 = y[selected_mono1] - yd[0]
    z_dist_1 = z[selected_mono1] - zd[0]
    
    r2_dist_1 = x_dist_1**2 + y_dist_1**2 + z_dist_1**2
    r_dist_1 = np.sqrt(r2_dist_1)
    
    
    x_dist_2 = x[selected_mono1-1] - xd[0]
    y_dist_2 = y[selected_mono1-1] - yd[0]
    z_dist_2 = z[selected_mono1-1] - zd[0]
    
    r2_dist_2 = x_dist_2**2 + y_dist_2**2 + z_dist_2**2
    r_dist_2 = np.sqrt(r2_dist_2)
    
    if(r_dist_1>=r_dist_2):
        selected_mono1 = selected_mono1-1
    
    return selected_mono1

def distance_check_2(x,y,z,xd,yd,zd,selected_mono2,i):
    x_dist_1 = x[selected_mono2] - xd[1]
    y_dist_1 = y[selected_mono2] - yd[1]
    z_dist_1 = z[selected_mono2] - zd[1]
    
    r2_dist_1 = x_dist_1**2 + y_dist_1**2 + z_dist_1**2
    r_dist_1 = np.sqrt(r2_dist_1)
    
    
    x_dist_2 = x[selected_mono2+1] - xd[1]
    y_dist_2 = y[selected_mono2+1] - yd[1]
    z_dist_2 = z[selected_mono2+1] - zd[1]
...
for i in tqdm(range(timesteps)):
    ...
    if(i>10000 and selected_mono1>=1 and selected_mono2<=498):
        fx,fy,fz = force_pull(fx,fy,fz,x,y,z,rc2)
...
new_path = r'D:\Study Material\College\MS Thesis\Code\Langevin Polymer\SPYDER\UNDERDAMPED\Latest\eqbm_pull_force500_dd1000_dp100_distchek_leaves2.xyz'
